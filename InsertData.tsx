export const InsertShopDetails = [
  {
    name: "Live Ice Cream by Harsh",
    category: "Ice Cream",
    slug: "live-ice-cream",
    imageURL:
      "https://drive.google.com/uc?id=1IJ7WWJa1R1HsPgHWT1Siady7rT81mWBd",
    rating: 0,
    products: [
      {
        name: "Oreo",
        price: 49,
        productImage:
          "https://drive.google.com/uc?id=1q8RsW3w7_KaeZnYh01rybzKtyxdgl4cm",
      },
      {
        name: "Roll",
        price: 55,
        productImage:
          "https://drive.google.com/uc?id=1rtLV0dRsCswin2KfdkmLUZYOLJvoqCJ8",
      },
      {
        name: "Rabadi Gulab Jamun",
        price: 79,
        productImage:
          "https://drive.google.com/uc?id=1kgZHqCD_zPUFeix6IYcC-4DuIOk4Q7w1",
      },
      {
        name: "Rocky Road",
        price: 89,
        productImage:
          "https://drive.google.com/uc?id=12d8HO6b5j4rWSMOb4Z4SQv7-LiLOoPoR",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Spicy Splash Zone by Sneha",
    category: "Food",
    slug: "panipuri-corner",
    imageURL:
      "https://drive.google.com/uc?id=1uB7_8kFBOwiniU6f37RvLV_xZOtGSZJe",
    rating: 0,
    products: [
      {
        name: "Pani Puri",
        price: 20,
        productImage:
          "https://drive.google.com/uc?id=1hjZ5oqpzVfMBJQDruum9Xm4w-OWPkmf7",
      },
      {
        name: "Sev Puri",
        price: 30,
        productImage:
          "https://drive.google.com/uc?id=1tXR9wv9MafnAKc12qJ0CVNd9aHgfdp7s",
      },
      {
        name: "Dahi Puri",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=1PV5TfVkdwrgKHsJYU3KCqkxxO5bnybDj",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Little Bites by Nidhi",
    category: "Food",
    slug: "little-bites",
    imageURL:
      "https://drive.google.com/uc?id=1IJ7WWJa1R1HsPgHWT1Siady7rT81mWBd",
    rating: 0,
    products: [
      {
        name: "Nuggets",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=1rUJ8xc6zVlvLxIG2nPF_W-6AshpU2--9",
      },
      {
        name: "Chicken Strips",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1ToU9HT0G5FQtweZnF2eqDWidgb33Elcr",
      },
      {
        name: "Spicy Chicken",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1iIXE4cifNPsiTA1G8CqWjz71c3JTvc3R",
      },
      {
        name: "Strawberry Juice",
        price: 10,
        productImage:
          "https://drive.google.com/uc?id=1ekIDc5VB14qQfC9hCar7cRqrJh-bO_Fe",
      },
      {
        name: "Chicken Paav",
        price: 25,
        productImage:
          "https://drive.google.com/uc?id=1go6Uqp5VeVZk_L9Cds53zv6JUxC-Dgj_",
      },
      {
        name: "Burger",
        price: 40,
        productImage:
          "https://drive.google.com/uc?id=1XmlqRTTQu1nNRQXpxAIUBb5Reb2aKXhj",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Chills by Kshitij",
    category: "Cold Drinks and Food",
    slug: "chills",
    imageURL:
      "https://drive.google.com/uc?id=1ijmEg3dMbb-tDcXzym4RS4AhC9Ktauam",
    rating: 0,
    products: [
      {
        name: "Virgin Mojito",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1ze87YMORrfN7vulMh_pyLJpX0_pK1NpK",
      },
      {
        name: "Lemonade",
        price: 25,
        productImage:
          "https://drive.google.com/uc?id=1LugA1JuXG9dilQlyClMEwMpxUWq7OAc1",
      },
      {
        name: "Blue Lagoon",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=18M-V-0wW_C1MPavvvU4xXSHUsKIKW3yz",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Chocolate Strawberry by Vedant",
    category: "Chocolate",
    slug: "chocolate-strawberry",
    imageURL:
      "https://drive.google.com/uc?id=1b6nORNPW_e97x8fvM26_wJ8-k7Un_tMp",
    rating: 0,
    products: [
      {
        name: "Chocolate Strawberry",
        price: 120,
        productImage:
          "https://drive.google.com/uc?id=1KymsCt1gDCQSBxbVrEVNeAg1GDL1vA1s",
      },
    ],
    startDate: 17,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Kankru Crunchies by Krutika",
    category: "Food & Drinks",
    slug: "kankru-crunchies",
    imageURL:
      "https://drive.google.com/uc?id=1-iLV8-bY_lekA6m0u7TiBanD5YJXm2Wr",
    rating: 0,
    products: [
      {
        name: "Biryani",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1Q7pqHKPL5mfXxXxqKH8KsvHz3UqIeIbM",
      },
      {
        name: "Pav Bhaji",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1loIx_Ypn3xOTY-q7xCQGvi_mLKFugwHZ",
      },
      {
        name: "Kokam Fizz",
        price: 40,
        productImage:
          "https://drive.google.com/uc?id=1uCAN4KygY--W59K9vOnyepU3_VcpTnyf",
      },
      {
        name: "Carrot Sandwich",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1rI4Nf6L8ILfBwm_k3iyRkrQoRHhAWezm",
      },
      {
        name: "Mayo Corn Sandwich",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1jx9eLK7k4FZxj-USRpXyKHoNBYEXM4fm",
      },
    ],
    startDate: 16,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Ice Cream Mania by Jenam",
    category: "Ice Cream",
    slug: "ice-cream-mania",
    imageURL:
      "https://drive.google.com/uc?id=1GlTiPXSzasLvoJHDMUWWqkkC0H-pPGvI",
    rating: 0,
    products: [
      {
        name: "Sitafal Kulfi",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=18afPQTh_HxW0Tn3obcQJl5ipfpqtqSjO",
      },
      {
        name: "Tender Coconut Kulfi",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=1cYU14Twa3f0nihpqSJAS5Ijz85Lp_oOI",
      },
      {
        name: "Real Mango Kulfi",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=1FGlDU0Y7fQpcvezwu-aMMT8xX31L0VjC",
      },
      {
        name: "Oreo Kulfi",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=1J2J6gpidIN8wIjJeYVP_pFxBNTmezrdA",
      },
      {
        name: "Choco Almond Kulfi",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=1oIVDwdHIT1rANLLu0QsQpi-PS9R8Vlo7",
      },
      {
        name: "Zafrani Pista",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=1lG0P4YRY40-_ZHkT9eDklpC4XAHXrLmA",
      },
      {
        name: "Chocobar",
        price: 20,
        productImage:
          "https://drive.google.com/uc?id=1qTiPCor-8B1Avdj4WqkCcgwosqco5w4i",
      },
      {
        name: "Butterscotch",
        price: 30,
        productImage:
          "https://drive.google.com/uc?id=1lOqiQxW7j9BH4kuaU-nwfUsy3sLniPA1",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Snack Attach By Chinmay",
    category: "Snacks",
    slug: "snack-attack",
    imageURL:
      "https://drive.google.com/uc?id=1uB7_8kFBOwiniU6f37RvLV_xZOtGSZJe",
    rating: 0,
    products: [
      {
        name: "Cheese Mayo Chips",
        price: 40,
        productImage:
          "https://drive.google.com/uc?id=1WkozFTB60HCqEQnXHv4a3RCQ5Q83RGyQ",
      },
      {
        name: "Tandoori Chicken Chips",
        price: 65,
        productImage:
          "https://drive.google.com/uc?id=1t_0Cz0BI5eM3tppCWU8dh9g7xKkas7LI",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Mocktails by Dheeraj",
    category: "Food",
    slug: "mocktails",
    imageURL:
      "https://drive.google.com/uc?id=1HEIjPaRfaS7DMemu3terbrdZJ0nScV20",
    rating: 0,
    products: [
      {
        name: "Mojito",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1MZTGKnbYvT0osQE3l9rbg6D96NNEDnCK",
      },
      {
        name: "Masala Mojito",
        price: 60,
        productImage:
          "https://drive.google.com/uc?id=1LxiZ_U22FNW6w0TmQ8RYITQLt6teCawB",
      },
      {
        name: "Kokam Masala",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1n6YNeAnHJOkOcy_QTm75SFhLjQ3n2uPT",
      },
      {
        name: "Blue Lagoon",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1fJUL4kTIYuOKKC2S-dZA5d7yho1okrrr",
      },
      {
        name: "Green Apple Lemonade",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1JFfJaveq5MkNCyEHNYpK7cJhRriQdGL5",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Pineresty by Sai",
    category: "Accessories",
    slug: "pinterestry",
    imageURL:
      "https://drive.google.com/uc?id=1GlTiPXSzasLvoJHDMUWWqkkC0H-pPGvI",
    rating: 0,
    products: [
      {
        name: "Shooting Star",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=1EyrT7s3Sm1qp0rlKFQbkMFBeIZEURJOF",
      },
      {
        name: "Bow phone charms",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=19fYjD3RWLhoXYLGvrg9uRGu0Qkod0sNy",
      },
      {
        name: "Blooming Blossom",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=1N4b417QS138HUOf1TPw3GEEQprD8FIqo",
      },
      {
        name: "Bow Keychain",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=1mUo5PZ7t4qKNeV1isfY83UgGwqQWdIcq",
      },
      {
        name: "Sunflower Keychain",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=1uNtxf4q5XtJ0Bsbkm2Knc_k7BVDxpEAX",
      },
      {
        name: "Mushroom Keychain",
        price: 150,
        productImage:
          "https://drive.google.com/uc?id=1EL7y6Mv8738axWA2tVJEVECOn9OnTvgB",
      },
      {
        name: "Octopus Keychain",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1-usWypw-QCauc9GKFMOutNECfwnxn9f9",
      },
      {
        name: "Strawberry Keychain",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=1kY6XbwnVbDU9gk_z7jJlTt504PKKm97D",
      },
      {
        name: "Pineapple Keychain",
        price: 90,
        productImage:
          "https://drive.google.com/uc?id=1QQb1RwMHqc2TcD5ISRteYaEV1VtTEnJn",
      },
      {
        name: "Shell Bracelets",
        price: 150,
        productImage:
          "https://drive.google.com/uc?id=1rDbWCdanZp0V5vDR4Dkdf0K3dTz8cTHj",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Totally Totes by Arya",
    category: "Bags",
    slug: "totally-totes",
    imageURL:
      "https://drive.google.com/uc?id=1uB7_8kFBOwiniU6f37RvLV_xZOtGSZJe",
    rating: 0,
    products: [
      {
        name: "Totes",
        price: 100,
        productImage:
          "https://drive.google.com/uc?id=1eRESYu2zDsgjkLJ1QGUWsQDVjmaiMSf9",
      },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "ADHD by Arya",
    category: "Food",
    slug: "adhd",
    imageURL:
      "https://drive.google.com/uc?id=1bUieKLO86OLZTEOjNiZg4q2ID-n4gKgn",
    rating: 0,
    products: [
      {
        name: "Product 2",
        price: 44,
        productImage:
          "https://drive.google.com/uc?id=1ckWntpoawUpqMKU0vStswz4k3SlOEQH2",
      },
      {
        name: "Product 3",
        price: 20,
        productImage:
          "https://drive.google.com/uc?id=1ckWntpoawUpqMKU0vStswz4k3SlOEQH2",
      },
    ],
    startDate: 14,
    endDate: 16,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Crispy Tales by Vinayak",
    category: "Food",
    slug: "mint-and-mix",
    imageURL:
      "https://drive.google.com/uc?id=1ojWV5YKS2ruYAMvZ_q-qwenPmSitifJm",
    rating: 0,
    products: [
      {
        name: "Chicken Biryani",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1GRKxpcQxg-mubr00GYBpSf1lY9ogjjlv",
      },
      // {
      //   name: "Manchurian Bhaji",
      //   price: 20,
      //   productImage:
      //     "https://drive.google.com/uc?id=1-lN9CiGr-BRBIoKxrFivnSwKIdPNWdpr",
      // },
      {
        name: "Chinese Bhel",
        price: 15,
        productImage:
          "https://drive.google.com/uc?id=1ncBaSGa_S9H6bNF52JCFcu3nEMhTEd68",
      },
      // {
      //   name: "Manchurian Bhel",
      //   price: 30,
      //   productImage:
      //     "https://drive.google.com/uc?id=1RTqBhpPN_MFJXnZY9kTRMWT8CJQ8bxOg",
      // },
    ],
    startDate: 18,
    endDate: 20,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Wrap it up by Shreya",
    category: "Sandwich",
    slug: "wrap-it-up",
    imageURL:
      "https://drive.google.com/uc?id=1HThr4teZfy4c-lH-6LtvP-6ipF-wsoxf",
    rating: 0,
    products: [
      {
        name: "Veg Sandwich",
        price: 35,
        productImage:
          "https://drive.google.com/uc?id=1I5v0b6F-8GW67iOaC60VoJWdq-G4zvfq",
      },
      {
        name: "Toast Sandwich",
        price: 40,
        productImage:
          "https://drive.google.com/uc?id=1lXR3fyAtk5dT8A1R7H2o4Z4_9zZobkB6",
      },
      {
        name: "Mayo Sandwich",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=17Be39Y4F8CUH_gdJBD-ZD1CNtqdVsd80",
      },
      {
        name: "Cheese Chilli toast Sandwich",
        price: 65,
        productImage:
          "https://drive.google.com/uc?id=1cCWkMWfmi_lGyJafCe-qZTFXO4GcXe2M",
      },
    ],
    startDate: 17,
    endDate: 17,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Flip & Dip by Muskaan",
    category: "Pancakes",
    slug: "flip-and-dip",
    imageURL:
      "https://drive.google.com/uc?id=1cwYYxHPduKzmiuNIGOG1uBCSeGwkY22f",
    rating: 0,
    products: [
      {
        name: "ButterScotch Pancakes",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1mkIBPyxDt121590Avci3pJIBXsUq3bgU",
      },
      {
        name: "Chocolate Pancakes",
        price: 60,
        productImage:
          "https://drive.google.com/uc?id=1XimNwX_4FlSYqC51FhpRz3ZB1zGSlIie",
      },
      {
        name: "Oreo in Butterscotch",
        price: 60,
        productImage:
          "https://drive.google.com/uc?id=18n1UNbZQRyLKbDgKMsiI7nJ-tOPsgwWZ",
      },
      {
        name: "Snowy Chocolate",
        price: 15,
        productImage:
          "https://drive.google.com/uc?id=1PanYKCY3AI71isaph_ys2aA-YQ58ot_r",
      },
      {
        name: "Cheeky Choco Chips",
        price: 15,
        productImage:
          "https://drive.google.com/uc?id=1XhzdOjfyYsa6d02Mh-ORRa0sp9Mr9FQd",
      },
      {
        name: "Rainbow Sprinkles",
        price: 10,
        productImage:
          "https://drive.google.com/uc?id=1rcRyx6dr2LQHr0BDdNnHvXj9ZtP5nhx-",
      },
      {
        name: "Erratic Oreo",
        price: 10,
        productImage:
          "https://drive.google.com/uc?id=1szrTB6SGCpdRGllIjiV6KCsWoH75iF5W",
      },
    ],
    startDate: 17,
    endDate: 18,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Carnival Cravings by Yukta",
    category: "Fries",
    slug: "carnival-cravings",
    imageURL:
      "https://drive.google.com/uc?id=1UamdOVFTUuyUWRiITc1XUz4ci7BshOPZ",
    rating: 0,
    products: [
      {
        name: "Classic Twister",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1FW_hIS1RZqTGSW55fTloQze5LH3eKuM5",
      },
      {
        name: "Mayo Twister",
        price: 55,
        productImage:
          "https://drive.google.com/uc?id=1Ir2JOPO7mNWMrBUFV2e5XIZXj6md1eZy",
      },
      {
        name: "BBQ Twister",
        price: 65,
        productImage:
          "https://drive.google.com/uc?id=1CRTeO-mGyOsncyPwcCdvuhH8K8knY1gw",
      },
      {
        name: "Cheese Twister",
        price: 65,
        productImage:
          "https://drive.google.com/uc?id=1R8APD6ij433IdDhLf7xYt03YGFxGyyjd",
      },
      {
        name: "Classic Fries",
        price: 30,
        productImage:
          "https://drive.google.com/uc?id=1pr4hI2e-j9G7cDhHM3sYLQ5HBXj9CwXU",
      },
      {
        name: "Periperi Fries",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1eXv2bqXFcmbWLF8QsVRvg1cSkkz3R-HD",
      },
    ],
    startDate: 17,
    endDate: 18,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Anaika Jewels",
    category: "jewels",
    slug: "anaika-jewels",
    imageURL:
      "https://drive.google.com/uc?id=1epbjkfMWT8wES0NrIUp-a6XoPX0zKoUc",
    rating: 0,
    products: [
      {
        name: "Jhumkas",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1yen6ks7o94wE9NVlbbqLWAy7tLJRnexW",
      },
    ],
    startDate: 17,
    endDate: 18,
    ratedUsers: [],
    ratingsArray: [],
  },
  {
    name: "Artistry Homebakes",
    category: "Food",
    slug: "artistry-homebakes",
    imageURL:
      "https://drive.google.com/uc?id=1WABfQKHczQrcSvMnxN1GeoyPYNX77LUL",
    rating: 0,
    products: [
      {
        name: "Veg Burger",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1lw_BZok5faCxjFWG2zm_nGlNgBFJSVMa",
      },
      {
        name: "Veg Double Tikki Burger",
        price: 80,
        productImage:
          "https://drive.google.com/uc?id=1cX39ZxQYV4Fq9uYdFp-OBJ5O4X-dnHup",
      },
      {
        name: "Veg Cheese Burger",
        price: 70,
        productImage:
          "https://drive.google.com/uc?id=1QYHgSt4y4pu_ktG5w6USkz3Yf1XL065W",
      },
      {
        name: "Normal Fries",
        price: 40,
        productImage:
          "https://drive.google.com/uc?id=1sroptU1Hx2YxTQ40YicC-SLoz17Orqx3",
      },
      {
        name: "Cupcakes",
        price: 30,
        productImage:
          "https://drive.google.com/uc?id=1O-PlplIZiA6-48adbs5NjK46JP9EOVs0",
      },
      {
        name: "Chocolate Pastry",
        price: 50,
        productImage:
          "https://drive.google.com/uc?id=1WKN7AFtF1anP1K66w4AQc3VrllTbTKGC",
      },
    ],
    startDate: 17,
    endDate: 18,
    ratedUsers: [],
    ratingsArray: [],
  },
];
